...
const double SINGLEPLAYER_INIT_TIME_MAP1 = 30;
const double SINGLEPLAYER_INIT_TIME_MAP2 = 70;
...
double singleplayer_extra_time(int lap_count, map_t *map) {
	...
	if (map_get_type(map) == MAP_ONE && lap_count < 20) {
		return (10.0 - 0.5 * (double)lap_count);
	}
	else if (map_get_type(map) == MAP_TWO && lap_count < 25) {
		return (40.0 - 2 * (double)lap_count);
	}
	return 0;
}
...
void server_init_client(server_t *server, client_t *client, size_t idx) {
	...
	for (size_t i = 0; i < body_count; ++i) {
		...
		switch (((collision_ref_t *)body_get_info(body))->body_type) {
			case WALL: {
				create_car_wall_collision(
					scene, car_get_elasticity_wall(car), car_get_body(car), body);
				break;
			}
			case GRASS: {
	...
	for (size_t i = 0; i < list_size(server->clients) - 1; ++i) {
		client_t *opponent_client = list_get(server->clients, i);;
