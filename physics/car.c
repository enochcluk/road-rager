...
const double TRACK_LENGTH_SCALE[] = { 0.075, 0.075 };
...
list_t *car_make_tracks(car_t *car, input_data_t *input) {
	...
	body_t *left_rear = body_init(
		left_rear_rect, INFINITY, color_set_opacity(car->tracks_color, opacity));
	list_add(objects, left_rear);

	body_t *right_rear = body_init(
		right_rear_rect, INFINITY, color_set_opacity(car->tracks_color, opacity));
